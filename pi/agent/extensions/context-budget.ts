...
const CONTEXT_CAPABILITY = "[CONTEXT CAPABILITY] Tool output is automatically bounded to protect context. When context usage is high, focused compaction preserves the active goal, success criteria, recent important results, errors, and file changes while removing redundant output.";
...
function latestGoal(ctx: ExtensionContext): GoalSnapshot | undefined {
  ...
  return entry?.type === "custom" && entry.data && typeof entry.data === "object" ? entry.data as GoalSnapshot : undefined;
}
...
function goalInstruction(ctx: ExtensionContext): string {
  ...
  const plan = goal.plan?.slice(0, 8).map((item, index) => `${index + 1}. ${item}`).join("\n") || "(plan not yet recorded)";
...
${goalInstruction(ctx)}`,
  ...
  pi.registerCommand("context-budget", {
    ...
    handler: async (args, ctx) => {
      ...
      const usage = ctx.getContextUsage();
      const usageText = usage?.percent === null || usage?.percent === undefined ? "unknown" : `${usage.percent.toFixed(0)}%`;
      ctx.ui.notify(`Context budget: ${enabled ? "on" : "off"} · tool cap ${maxResultChars.toLocaleString()} chars · auto-compact at ${compactPercent}% · current ${usageText}`, "info");
    },
  });

  pi.on("agent_end", (_event, ctx) => requestCompaction(ctx, "Context usage crossed the configured threshold after an agent run."));
