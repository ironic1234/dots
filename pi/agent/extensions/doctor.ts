...
const DOCTOR_CAPABILITY = "[DOCTOR CAPABILITY] Use /doctor for a read-only health check of Pi extensions, configuration, memory storage, Robinhood auth/catalog state, Git, and LSP servers.";
const LSP_COMMANDS = [
  "clangd",
  "pylsp",
  "gopls",
...
async function collectChecks(pi: ExtensionAPI, ctx: ExtensionCommandContext): Promise<Check[]> {
  ...
  const extensionChecks = await Promise.all(REQUIRED_EXTENSIONS.map(async (name): Promise<Check> => ({
  ...
  const checks: Check[] = [
    ...
    },
    {
      ...
      detail: !await exists(storePath) ? `ready to create: ${storePath}` : store ? `${storePath} is valid JSON` : `invalid JSON: ${storePath}`,
    },
    {
      label: "memory tool",
      status: allTools.includes("memory") ? "ok" : "fail",
      detail: allTools.includes("memory") ? (activeTools.has("memory") ? "loaded and active" : "loaded but inactive") : "not loaded",
    },
    {
      label: "question tool",
      status: allTools.includes("question") ? "ok" : "fail",
      detail: allTools.includes("question") ? (activeTools.has("question") ? "loaded and active" : "loaded but inactive") : "not loaded",
    },
    {
      label: "goal tool",
    ...
    },
    {
      ...
      status: oauth?.tokens && typeof oauth.tokens === "object" && typeof (oauth.tokens as JsonObject).access_token === "string" ? "ok" : "warn",
      detail: oauth?.tokens && typeof oauth.tokens === "object" && typeof (oauth.tokens as JsonObject).access_token === "string" ? "token present (secret hidden)" : "authentication required",
    },
    {
      label: "Robinhood catalog",
      status: allTools.some((name) => name.startsWith("robinhood_") && name !== "robinhood_search_tools") ? "ok" : "warn",
  ...
  const lspChecks = await Promise.all(LSP_COMMANDS.map(async (command): Promise<Check> => {
...
function formatReport(checks: Check[]): string {
  const highest = checks.reduce<CheckStatus>((current, check) => statusRank(check.status) > statusRank(current) ? check.status : current, "ok");
