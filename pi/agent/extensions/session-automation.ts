...
const SESSION_CAPABILITY = "[SESSION CAPABILITY] Unnamed sessions are named from their first meaningful request or active goal. Use /session-label <label> (or /label) to name the session and bookmark the current conversation branch.";
...
function latestGoalState(ctx: ExtensionContext): GoalState | undefined {
  for (const entry of [...branchEntries(ctx)].reverse()) {
    if (entry.type === "custom" && entry.customType === "goal-mode-state" && entry.data && typeof entry.data === "object") {
...
export default function sessionAutomationExtension(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("session-label", {
    ...
    handler: async (args, ctx) => {
      ...
      const entry = latestEntry(ctx);
      if (!label) {
        ctx.ui.notify(`Session: ${pi.getSessionName() ?? "(unnamed)"}\nCurrent branch label: ${entry?.id ? ctx.sessionManager.getLabel(entry.id) ?? "(none)" : "(no entry)"}`, "info");
        return;
      }
