...
function runWithTimeout(cmd, args, timeoutMs, label) {
  return new Promise((resolve, reject) => {
    execFile(cmd, args, { timeout: timeoutMs, maxBuffer: 512 * 1024 * 1024, encoding: 'utf8' }, (error, stdout, stderr) => {
      if (error) {
        if (error.killed) {
...
async function runMutool(sources, options) {
  ...
  for (const source of sources) {
    ...
    try {
      if (source.kind === 'url') {
        throw new Error('mutool engine cannot fetch URLs (this MuPDF build has no HTTP support); use --engine mcp or download the PDF locally');
      }

      if (options.include_metadata || options.include_page_count) {
        const infoText = await runWithTimeout('mutool', ['info', source.value], options.timeoutMs, 'mutool info');
      ...
      if (options.renderDir) {
        ...
        const stem =
          path
            .basename(source.value, path.extname(source.value))
            .replace(/[^A-Za-z0-9_-]/g, '_') || 'page';
...
function chooseEngine(options, sources) {
  ...
  if (large) {
    console.error('[pdf-reader-cli] auto engine: large file detected, using mutool for extraction (MCP has a text-size budget).');
  }
  ...
  try {
    ...
    try {
      ...
      if (options.engine === 'auto' && retryable) {
        console.error('[pdf-reader-cli] MCP text extraction exceeded the server budget; retrying with mutool engine.');
        await runMutool(resolved, options);
      } else {
        throw error;
      }
